...
jQuery(function() {
	...
	"use strict";
		...
		$('.editprofile-toggle').each(function(){
			if( $('h4',this).hasClass('active') ){
				$(this).find('.editprofile-toggle-content').show();
			}
		});
		$('.editprofile-toggle h4').on("click",function() {
			if( $(this).hasClass('active') ){
				$(this).removeClass('active');
				$(this).next('.editprofile-toggle-content').slideUp();
			} else {
				$(this).addClass('active');
				$(this).next('.editprofile-toggle-content').slideDown();
			}
		});
		...
		$.fn.modal.defaults.spinner = $.fn.modalmanager.defaults.spinner = '<div class="loading-spinner" style="width: 60px; margin-left: -30px; margin-top: -30px;>' +
			'<div class="modal-ajax-loading">' +
				'<img src="theme/assets/images/ajax-loading.gif" alt="image" />' +
			'</div>' +
		'</div>';
		...
		$(document).on('click', '.btn-ajax-login,.login-box-box-action a' ,function(){
			...
			setTimeout(function(){
				 $modal.load('student/page/signin', '', function()
				 {
					$modal.modal();
				});
			}, 100);
		...
		$(document).on('click', '.btn-ajax-register,.register-box-box-action a' ,function()
		{
			...
			setTimeout(function(){
				 $modalRegister.load('student/page/signup', '', function()
				 {
					$modalRegister.modal();
				});
			}, 100);
		...
		$(document).on('click', '.btn-ajax-forgot-password,.login-box-link-action a' ,function(){
			...
			setTimeout(function(){
				 ...
				 $modalForgotPassword.load('student/page/forgot', '', function(){
