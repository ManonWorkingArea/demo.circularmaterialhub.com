...
(function (window){
	...
	var FWDUVPlayer = function(props){
		...
		self.setupInfo = function(){
			...
			self.info_do = new FWDUVPInfo(self, self.warningIconPath_str);
		};		
		...
		this.youtubeReadyHandler = function(e){
			...
			if(self.ytb_do.hasBeenCreatedOnce_bl){
				...
				if(self.videoSourcePath_str.indexOf(".") == -1) self.updateAds(0, true);
				return;
			}
			
			//console.log("youtube ready - is vimeo ready =  " + self.isVimeoReady_bl)
			
			if(self.useVimeo_bl && !self.isVimeoReady_bl) return;
			
			if(self.useYoutube_bl && self.useVimeo_bl){
				if(self.isYoutubeReady_bl && self.isVimeoReady_bl) self.setupNormalVideoPlayers();
			}else if(self.useYoutube_bl){
				if(self.isYoutubeReady_bl) self.setupNormalVideoPlayers();
			}else if(self.useVimeo_bl){
				if(self.isVimeoReady_bl) self.setupNormalVideoPlayers();
			}
		...
		this.setupVimeoPlayer = function(){
			...
			self.vimeo_do.addListener(FWDUVPVimeoScreen.INIT_ERROR, self.vimeoInitErrorHandler);
		...
		this.vimeoReadyHandler = function(e){
			...
			//console.log("vimeo ready - is youtube ready =  " + self.isYoutubeReady_bl)
		...
		this.updatePlaylist = function(){
			if((self.useYoutube_bl && self.useVimeo_bl) && (!self.isYoutubeReady_bl || !self.isVimeoReady_bl)){
				setTimeout(self.updatePlaylist, 50);
				return;
			}else if(self.useYoutube_bl && !self.isYoutubeReady_bl){
				setTimeout(self.updatePlaylist, 50);
				return;
			}else if(self.useVimeo_bl && !self.isVimeoReady_bl){
				setTimeout(self.updatePlaylist, 50);
				return;
			}
	    	...
	    	//console.log(self.data.playlist_ar)
		...
		this.loadInternalPlaylist = function(){
			...
			if(self.preloader_do) self.preloader_do.show(true);
		...
		this.infoWindowHideCompleteHandler = function(){
			...
			if(self.controller_do && !self.isMobile_bl){
				...
				self.controller_do.infoButton_do.setNormalState();
			}
		};
		...
		this.controllerChangeYoutubeQualityHandler = function(e){
			...
			if(self.videoType_str == FWDUVPlayer.YOUTUBE){
				...
				self.setSource(self.data.playlist_ar[self.id].videoSource[self.data.playlist_ar[self.id].startAtVideo]["source"]);
		...
		this.videoScreenSafeToScrubbHandler = function(){
			if(self.controller_do){
				...
				if(self.controller_do.ytbQualityButton_do && !self.isAdd_bl) self.controller_do.ytbQualityButton_do.enable();
		...
		this.videoScreenPlayHandler = function(){
			...
			if(self.playAtTime_bl) self.scrubbAtTime(self.data.playlist_ar[self.id].scrubAtTimeAtFirstPlay);
		...
		this.videoScreenPauseHandler = function(){
			...
			if(self.largePlayButton_do &&  !self.isAdd_bl && self.videoType_str != FWDUVPlayer.VIMEO  && !self.data.showAnnotationsPositionTool_bl) self.largePlayButton_do.show();
		...
		this.videoScreenUpdateTimeHandler = function(e, e2, e3){
			...
			if(self.subtitle_do) self.subtitle_do.updateSubtitle(parseInt(e.seconds));
		...
		this.setPosterSource = function(path){
			...
			if((self.videoSourcePath_str.indexOf(".") == -1 && self.useYoutube_bl && self.isMobile_bl)
			   || (self.videoSourcePath_str.indexOf("vimeo.com") == -1 && self.useVimeo_bl && self.isMobile_bl)){
			...
			if(!this.isAdd_bl && self.curAddData){
				...
				for(var i=0; i<self.curAddData.length; i++){
					if(duration >= self.curAddData[i].timeStart && duration <= (self.curAddData[i].timeStart + 1) 
						...
						if(self.videoType_str != FWDUVPlayer.IMAGE && !self.isMobile_bl && !self.loadAddFirstTime_bl || self.data.autoPlay_bl) self.play();
		...
		this.setSource = function(source, overwrite){
			...
			*/
			...
			if(self.videoType_str == FWDUVPlayer.YOUTUBE){
				...
				if(self.controller_do) self.controller_do.addYtbQualityButton();
				self.resizeHandler(false, true);
				if(self.getVideoSource()) self.dispatchEvent(FWDUVPlayer.UPDATE_VIDEO_SOURCE);
				return;
			}
	  ...
	  //#############################################//
		...
		setTimeout(FWDUVPlayer.checkIfHasYoutube, 500);
	};
	
	//############################################//
	/* setup youtube api */
	//############################################//
	FWDUVPlayer.setupAllInstances = function(){
		if(FWDUVPlayer.areInstancesCreated_bl) return;
		...
		if(FWDUVPlayer.videoStartBehaviour != "pause" 
			...
			&& FWDUVPlayer.videoStartBehaviour != "none"
		){
			FWDUVPlayer.videoStartBehaviour = "pause";
		}
		if(FWDUVPUtils.isMobile_bl) FWDUVPlayer.videoStartBehaviour = "stop";

		FWDUVPlayer.areInstancesCreated_bl = true;
		...
		if(embedTest){
			for(var i=0; i<tt; i++){
				...
				if(video.props.instanceName == embedTest){
					FWDUVPlayer.isEmbedded_bl = true;
					//video.init();
					return;
				}
			}
		...
		for(var i=0; i<tt; i++){
			video = FWDUVPlayer.instaces_ar[i];
			prevVideo = FWDUVPlayer.instaces_ar[i - 1];
	...
	FWDUVPlayer.checkIfHasYoutube = function(){
		if(FWDUVPlayer.checkIfHasYoutube_bl) return;
		FWDUVPlayer.checkIfHasYoutube_bl = true;
		...
		if(hasYoutube_bl){
			...
			setTimeout(FWDUVPlayer.setupAllInstances, 500);
		}
	};
	...
	FWDUVPlayer.setupYoutubeAPI = function(){
		...
		if(typeof YT != "undefined"){
			...
			if(window.onYouTubeIframeAPIReady){
				...
				setTimeout(FWDUVPlayer.setupAllInstances, 1000);
			}
		}
	};
	...
	FWDUVPlayer.countInstances = 1;
	FWDUVPlayer.setupMainInstance = function(){
		
		if(FWDUVPlayer.areMainInstancesInitialized_bl) return;
		FWDUVPlayer.areMainInstancesInitialized_bl = true;
		
		
		setTimeout(function(){
			for(var i=0; i<FWDUVPlayer.instaces_ar.length; i++){
			FWDUVPlayer.instaces_ar[i].init();
			//if(FWDUVPlayer.countInstances < FWDUVPlayer.instaces_ar.length) FWDUVPlayer.setupMainInstance();
			///FWDUVPlayer.countInstances++;
			}
		}, FWDUVPlayer.countInstances * 100);
	};
