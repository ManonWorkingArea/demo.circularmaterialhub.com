...
(function (window){
	var FWDUVPCategoriesThumb = function(
		...
		this.setImage = function(image){
			...
			if(self.effectImage_do){
				FWDAnimation.to(self.effectImage_do, .8, {
					x:self.imageFinalX, 
					y:self.imageFinalY, 
					ease:Expo.easeInOut});
			}
			...
			if(self.isMobile_bl){
				if(self.hasPointerEvent_bl){
					...
					self.screen.addEventListener("pinterout", self.onMouseOut);
		...
		this.resizeImage = function(animate){
			...
			if(self.effectImage_do){
				FWDAnimation.killTweensOf(self.effectImage_do);
		...
		this.setNormalState = function(animate){
			...
			if(self.thumbnailSelectedType_str == "threshold" || self.thumbnailSelectedType_str == "blackAndWhite"){
				if(animate){
					FWDAnimation.to(self.effectImage_do, 1, {alpha:.01, ease:Quart.easeOut});
				}else{
					self.effectImage_do.setAlpha(.01);
				}
			}else if(self.thumbnailSelectedType_str == "opacity"){
				if(animate){
					FWDAnimation.to(self.normalImage_do, 1, {alpha:1, ease:Quart.easeOut});
				}else{
					self.normalImage_do.setAlpha(1);
				}
			}
		...
		this.setSelectedState = function(animate, overwrite){
			...
			if(self.thumbnailSelectedType_str == "threshold" || self.thumbnailSelectedType_str == "blackAndWhite"){
				if(animate){
					FWDAnimation.to(self.effectImage_do, 1, {alpha:1, ease:Expo.easeOut});
				}else{
					self.effectImage_do.setAlpha(1);
				}
			}else if(self.thumbnailSelectedType_str == "opacity"){
				if(animate){
					FWDAnimation.to(self.normalImage_do, 1, {alpha:.3, ease:Expo.easeOut});
				}else{
					self.normalImage_do.setAlpha(.3);
				}
			}
