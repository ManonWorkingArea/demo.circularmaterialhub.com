...
(function (window){
	...
	var FWDUVPPlaylist = function(
		...
		/* update playlist */
		//#################################//
		this.updatePlaylist = function(playlist, catId, id, playlistName){
			//console.log(self.catId + "update " + catId)
			
			if(self.catId == catId) return;
			...
			FWDAnimation.to(self.mainHolder_do, .8, {x:0, y:0, ease:Expo.easeInOut});
		...
		this.showNothingFound = function(){
			...
			FWDAnimation.to(self.noSearchFound_do, .1, {alpha:1, yoyo:true, repeat:4});
		};
		...
		this.hideNothingFound = function(){
			...
			FWDAnimation.killTweensOf(self.noSearchFound_do);
			self.noSearchFound_do.setVisible(false);
		};
		...
		this.setupPrevButton = function(){
			...
			self.prevButton_do = new FWDUVPSimpleButton(self.prevN_img, data.prevSPath_str, undefined, true, 
		...
		this.setupNextButton = function(){
			FWDUVPSimpleButton.setPrototype();
			self.nextButton_do = new FWDUVPSimpleButton(self.nextN_img, data.nextSPath_str, undefined, true, 
			data.useHEXColorsForSkin_bl,
		    data.normalButtonsColor_str,
		    data.selectedButtonsColor_str);
		...
		this.setupMobileScrollbar = function(){
			if(self.hasPointerEvent_bl){
				self.mainThumbsHolder_do.screen.addEventListener("pointerdown", self.scrollBarTouchStartHandler);
			}else{
		...
		this.setupScrollbar = function(){
			...
			self.middleImage.src = data.scrDragMiddlePath_str;
			if(self.useHEXColorsForSkin_bl){
			...
			self.bottomImage.src = data.scrDragMiddlePath_str;
			if(self.useHEXColorsForSkin_bl){
				...
				self.scrHandlerBottom_do.getStyle().background = "url('" + data.scrDragBottomPath_str + "')";
			}
			self.scrHandlerBottom_do.setWidth(self.scrWidth);
			...
			self.scrHandlerLinesS_img.src = data.scrLinesSPath_str;
			if(self.useHEXColorsForSkin_bl){
		...
		this.updateScrollBarHandlerAndContent = function(animate, overwrite){
			...
			if(self.lastThumbnailFinalY != self.thumbnailsFinalY){
				FWDAnimation.killTweensOf(self.thumbsHolder_do);
				if(animate){
					FWDAnimation.to(self.thumbsHolder_do, .5, {y:self.thumbnailsFinalY, ease:Quart.easeOut});
				}else{
					self.thumbsHolder_do.setY(self.thumbnailsFinalY);
				}
			}
		...
		this.hideAndShow = function(animate){
			if(self.position_str == "bottom"){
				...
				FWDAnimation.to(self.mainHolder_do, .8, {x:0, ease:Expo.easeInOut});
			}
		};
		...
		this.show = function(animate){
			...
			if(!FWDAnimation.isTweening(self.mainHolder_do)) self.hide(false);
			if(animate){
				if(self.position_str == "bottom"){
					FWDAnimation.to(self.mainHolder_do, .8, {y:0, ease:Expo.easeInOut});
				}else{
		...
		self.updateHEXColors = function(normalColor_str, selectedColor_str){
			...
			try{
				FWDUVPUtils.changeCanvasHEXColor(self.scrLinesN_img, self.mainhandlerN_canvas, self.selectedColor_str);
				
				var scrubbelinesSImage_img = FWDUVPUtils.changeCanvasHEXColor(self.scrHandlerLinesS_img, self.scrubberLines_s_canvas.canvas, self.selectedColor_str, true);
				self.scrHandlerLinesS_do.getStyle().background = "url('" + scrubbelinesSImage_img.src + "') repeat-x";
				FWDUVPUtils.changeCanvasHEXColor(self.scrDragTop_img, self.mainScrubberDragTop_canvas, self.normalColor_str);
				
				var middleImage_img = FWDUVPUtils.changeCanvasHEXColor(self.middleImage, self.scrubberDragMiddle_canvas.canvas, self.normalColor_str, true);
				self.scrHandlerMiddle_do.getStyle().background = "url('" + middleImage_img.src + "') repeat-y";
				
				var bottomImage_img = FWDUVPUtils.changeCanvasHEXColor(self.bottomImage, self.scrubberDragBottom_canvas.canvas, self.normalColor_str, true);
				self.scrHandlerBottom_do.getStyle().background = "url('" + bottomImage_img.src + "') repeat-y";
			}catch(e
			){}			
			
			
			if(self.nextButton_do) self.nextButton_do.updateHEXColors(normalColor_str, selectedColor_str);
			if(self.prevButton_do) self.prevButton_do.updateHEXColors(normalColor_str, selectedColor_str);
			if(self.loopButton_do) self.loopButton_do.updateHEXColors(normalColor_str, selectedColor_str);
			if(self.shuffleButton_do) self.shuffleButton_do.updateHEXColors(normalColor_str, selectedColor_str);
			
			if(self.input_do) self.input_do.getStyle().color = self.normalColor_str;
			if(self.playlistName_do) self.playlistName_do.getStyle().color = self.normalColor_str;
