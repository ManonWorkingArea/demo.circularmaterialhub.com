/* FWDUVPSubtitle */
(function (window){
var FWDUVPSubtitle = function(parent, data){
		...
		this.setSizeOnce = function(){
			this.setWidth(parent.tempVidStageWidth);
			this.text_do.setWidth(parent.tempVidStageWidth);
		}
		...
		this.onLoad = function(e){
			...
			if(self.xhr.readyState == 4){
				if(self.xhr.status == 404){
					self.dispatchEvent(FWDUVPData.LOAD_ERROR, {text:"Subtitle file path is not found: <font color='#FF0000'>" + self.sourceURL_str + "</font>"});
				}else if(self.xhr.status == 408){
					self.dispatchEvent(FWDUVPData.LOAD_ERROR, {text:"Loadiong subtitle file file request load timeout!"});
			...
			self.dispatchEvent(FWDUVPSubtitle.LOAD_COMPLETE);
		};
		...
		this.onError = function(e){
			...
			self.dispatchEvent(FWDUVPSubtitle.LOAD_ERROR, {text:"Error loading subtitle file : <font color='#FF0000'>" + self.sourceURL_str + "</font>."});
		};
		...
		self.parseSubtitle = function(file_str){
		    ...
		    for(s in srt_) {
		        ...
		        if(st.length >=2) {
		            ...
		            self.subtitiles_ar[cont].startDuration =  FWDUVPSubtitle.getDuration(i);
		            self.subtitiles_ar[cont].endDuration = FWDUVPSubtitle.getDuration(o);
		            self.subtitiles_ar[cont].text = "<p class='UVPSubtitle'>" + t + "</p>";
		        }
		...
		this.position = function(animate){
			...
			if(parent.tempPlaylistPosition_str == "bottom"  || !parent.showPlaylistButtonAndPlaylist_bl){
				scale = Math.min(2, parent.tempVidStageWidth/(parent.maxWidth -  parent.spaceBetweenControllerAndPlaylist));
			}else{
				scale = Math.min(2, parent.tempVidStageWidth/(parent.maxWidth - parent.playlistWidth - parent.spaceBetweenControllerAndPlaylist));
			}
			...
			if(parent.controller_do){
				if(parent.controller_do.isShowed_bl){
					finalY = parseInt(parent.vidStageHeight - parent.controller_do.h - textHeight);
				}else{
					finalY = parseInt(parent.vidStageHeight - textHeight - 10);
				}	
			}else{
				finalY = parseInt(parent.vidStageHeight - textHeight);
			}
