...
(function(window){
	...
	var FWDUVPVimeoScreen = function(parent, volume){
		...
		this.initError = function(){
			self.dispatchEvent(FWDUVPVimeoScreen.INIT_ERROR, {error:"Error loading the Vimeo video player!"});
		};
		...
		this.setupVideo = function(){
			if(self.vimeolayer) return;
			self.iframe_do = new FWDUVPDisplayObject("IFRAME");
			...
			self.blackOverlay_do = new FWDUVPDisplayObject("div");
		...
		this.readyHandler = function(){
			...
			if(self.isReady_bl){
				...
				if(parent.videoType_str == FWDUVPlayer.VIMEO) self.setX(0);
				if(parent.data.autoPlay_bl) parent.play();
			
				return;
			}
			self.isReady_bl = true;
			self.dispatchEvent(FWDUVPVimeoScreen.READY);
		};
		...
		this.playHandler = function(){
			//if(!self.isSafeToBeControlled_bl){
				...
				self.startToUpdate();
				self.dispatchEvent(FWDUVPVimeoScreen.SAFE_TO_SCRUBB);
			//}
			self.dispatchEvent(FWDUVPVimeoScreen.PLAY);
		};
		
		this.loadProgressHandler = function(e){
			if(self.isShowed_bl) return;
			self.dispatchEvent(FWDUVPVimeoScreen.LOAD_PROGRESS, {percent:e.percent});
		};
		
		this.pauseHandler = function(){
			self.isPlaying_bl = false;
			self.dispatchEvent(FWDUVPVimeoScreen.PAUSE);
			self.stopToUpdate();
		};
		...
		this.finishHandler = function(){
			...
			self.dispatchEvent(FWDUVPVimeoScreen.PLAY_COMPLETE);
		};
		...
		this.play = function(overwrite){
			...
			if(parent.prevVideoType_str != FWDUVPlayer.VIMEO) dl = 800;
		...
		this.updateVideo = function(){
			...
			self.dispatchEvent(FWDUVPVimeoScreen.UPDATE_TIME, {curTime:curTime , totalTime:totalTime, seconds:parseInt(self.getCurrentTime())});
		};	
		...
		this.stop = function(addEvents){
			...
			if(!addEvents){
				self.stopVideo();
				self.dispatchEvent(FWDUVPVimeoScreen.STOP);
				self.dispatchEvent(FWDUVPVimeoScreen.LOAD_PROGRESS, {percent:0});
				self.dispatchEvent(FWDUVPVimeoScreen.UPDATE_TIME, {curTime:"00:00" , totalTime:"00:00"});
			}
	...
	FWDUVPVimeoScreen.INIT_ERROR = "initError";
