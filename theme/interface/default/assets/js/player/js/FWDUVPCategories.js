...
var FWDUVPCategories = function(data, parent){
		...
		this.mouseWheelDumyHandler = function(e){	
			var thumb;
			if(FWDAnimation.isTweening(self.mainHolder_do)){
				if(e.preventDefault){
					e.preventDefault();
				}
				return false;
		...
		this.setupButtons = function(){
			...
			self.closeButton_do = new FWDUVPSimpleButton(self.catCloseN_img, data.catCloseSPath_str, undefined,
					true,
			...
			FWDUVPSimpleButton.setPrototype();
			self.nextButton_do = new FWDUVPSimpleButton(self.catNextN_img, data.catNextSPath_str, undefined,
					true,
					data.useHEXColorsForSkin_bl,
					data.normalButtonsColor_str,
					self.selectedButtonsColor_str);
			...
			FWDUVPSimpleButton.setPrototype();
			self.prevButton_do = new FWDUVPSimpleButton(self.catPrevN_img, data.catPrevSPath_str, undefined,
					true,
					data.useHEXColorsForSkin_bl,
					data.normalButtonsColor_str,
					self.selectedButtonsColor_str);
		...
		this.setupMobileMove = function(){	
			if(self.hasPointerEvent_bl){
				self.screen.addEventListener("pointerdown", self.mobileDownHandler);
			}else{
		...
		this.mobileDownHandler = function(e){
			...
			if(self.hasPointerEvent_bl){
				window.addEventListener("pointerup", self.mobileUpHandler);
				window.addEventListener("pointermove", self.mobileMoveHandler);
			}else{
		...
		this.mobileUpHandler = function(e){
			...
			if(self.hasPointerEvent_bl){
				window.removeEventListener("pointerup", self.mobileUpHandler);
				window.removeEventListener("pointermove", self.mobileMoveHandler);
			}else{
		...
		this.show = function(id){
			...
			if(self.isMobile_bl){
				...
				FWDAnimation.to(self.mainHolder_do, .8, {y:0, delay:.4, ease:Expo.easeInOut});
			}else{
		...
		this.updateHEXColors = function(normalColor_str, selectedColor_str){
			...
			self.nextButton_do.updateHEXColors(normalColor_str, self.selectedColor_str);
			self.prevButton_do.updateHEXColors(normalColor_str, self.selectedColor_str);
		}
